    ...
    <div className="relative w-full aspect-square max-w-sm mx-auto select-none">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="w-28 h-28 rounded-3xl bg-gradient-to-br from-[#1e2d5f] to-[#0c1a2e] shadow-[0_0_60px_rgba(76,105,228,0.3)] flex items-center justify-center">
          <Code2 className="w-14 h-14 text-[#4c69e4]" strokeWidth={1.5} />
        </div>
      </div>

      {/* Orbiting nodes */}
      {[
        { icon: Shield,     label: "OWASP ASVS",  angle:   0, textColor: "text-[#4c69e4]"   },
        { icon: Smartphone, label: "Mobile",       angle:  60, textColor: "text-emerald-400" },
        { icon: Plug,       label: "API Security", angle: 120, textColor: "text-sky-400"     },
        { icon: Bot,        label: "AI Products",  angle: 180, textColor: "text-amber-400"   },
        { icon: Layers,     label: "Legacy Migr.", angle: 240, textColor: "text-purple-400"  },
        { icon: Shield,     label: "Architecture", angle: 300, textColor: "text-red-400"     },
      ...
      <div className="max-w-[1200px] mx-auto grid md:grid-cols-2 gap-12 items-center relative z-10">
        ...
        <div className="text-center md:text-left">
          ...
          <h1 className="text-5xl md:text-[3.5rem] font-bold font-manrope text-[#0c1a2e] leading-tight mb-6">
            ...
            <span className="text-[#4c69e4]">Development</span>
          </h1>

          <p className="text-lg text-slate-600 font-inter mb-8 max-w-lg leading-relaxed">
            We build software with security as a first-class requirement — hardened by design,
            not by patching. Every application we ship is engineered with the same adversarial
            mindset our penetration testers bring to the systems they break.
          </p>

          {/* Trust chips */}
          <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-8">
            {["OWASP ASVS", "OWASP MASVS", "NIST SSDF", "PCI-DSS v4.0", "CERT-In Compliant"].map((badge) => (
              <span
                key={badge}
