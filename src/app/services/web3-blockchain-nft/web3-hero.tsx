    ...
    <div className="relative w-full aspect-square max-w-sm mx-auto select-none">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="w-28 h-28 rounded-3xl bg-gradient-to-br from-[#1e2d5f] to-[#0c1a2e] shadow-[0_0_60px_rgba(76,105,228,0.3)] flex items-center justify-center">
          <FileCode className="w-14 h-14 text-[#4c69e4]" strokeWidth={1.5} />
        </div>
      </div>

      {/* Orbiting nodes */}
      {[
        { icon: MessageSquare, label: "NFT Chat",      angle:   0, textColor: "text-[#4c69e4]"   },
        { icon: FileCode,      label: "Smart Contract",angle:  51, textColor: "text-emerald-400" },
        { icon: Image,         label: "NFT Platform",  angle: 102, textColor: "text-sky-400"     },
        { icon: Zap,           label: "DeFi Security", angle: 154, textColor: "text-amber-400"   },
        { icon: KeyRound,      label: "Key Mgmt",      angle: 205, textColor: "text-purple-400"  },
        { icon: Lock,          label: "Token Gate",    angle: 257, textColor: "text-red-400"     },
        { icon: Search,        label: "Forensics",     angle: 308, textColor: "text-teal-400"    },
      ...
      <svg className="absolute inset-0 w-full h-full opacity-10" viewBox="0 0 100 100">
        {[0, 51, 102, 154, 205, 257, 308].map((angle) => {
          const rad = (angle * Math.PI) / 180;
          const r = 42;
          const x = 50 + r * Math.sin(rad);
          const y = 50 - r * Math.cos(rad);
...
export function Web3BlockchainHero() {
      ...
      <div className="max-w-[1200px] mx-auto grid md:grid-cols-2 gap-12 items-center relative z-10">
        ...
        <div className="text-center md:text-left">
          ...
          <h1 className="text-5xl md:text-[3.5rem] font-bold font-manrope text-[#0c1a2e] leading-tight mb-6">
            Web3 & <br />
            <span className="text-[#4c69e4]">Blockchain Security</span>
          </h1>

          <p className="text-lg text-slate-600 font-inter mb-8 max-w-lg leading-relaxed">
            Smart contracts, NFT platforms, and token-gated systems built with blockchain-native
            threat models — formal verification, institutional key management, and DeFi economic
            exploit modelling before a single dollar of TVL is at risk.
          </p>

          {/* Trust chips */}
          <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-8">
            {["Formal Verification", "Solidity · Rust", "ETH · SOL · Polygon", "CVSS Audit Report", "Zero Post-Deploy Breaches"].map((badge) => (
              <span
                key={badge}
        ...
        <div className="flex justify-center">
          <div className="bg-white/60 backdrop-blur-sm border border-slate-200 rounded-3xl p-6 shadow-[0_8px_40px_rgba(76,105,228,0.08)] w-full max-w-md">
            {!lottieError ? (
              <LottieAnimation
                src={getCdnUrl("lottie/web_testing.json")}
