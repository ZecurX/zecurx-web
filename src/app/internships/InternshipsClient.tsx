...
function groupPlansByRole(plans: Plan[]): GroupedPlan[] {
  ...
  for (const plan of plans) {
    const match = plan.name.match(
      /^(.+?)\s*(?:Internship\s*)?\((\d+)\s*Months?\)$/i,
    );
    if (!match) continue;
...
function InternshipCard({
  ...
  const [selectedDuration, setSelectedDuration] = useState<string>(
    group.variants.find((v) => v.months === 6)?.duration ||
      group.variants[group.variants.length - 1]?.duration ||
      "6months",
  );

  const currentVariant =
    group.variants.find((v) => v.duration === selectedDuration) ||
    group.variants[0];
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="max-w-4xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-border/50 border border-foreground/10 text-xs font-medium text-muted-foreground backdrop-blur-md mb-8">
              ...
              <span>
                <HeroWords>Live Internship Program</HeroWords>
              </span>
            </div>

            <h1 className="text-6xl md:text-8xl font-bold text-foreground mb-8 tracking-tighter">
              <HeroWords delay={heroEnd(3)}>Launch Your Career</HeroWords>{" "}
              <br />
              <HeroWords delay={heroEnd(6)}>with Real Experience</HeroWords>
            </h1>
            <p className="text-xl md:text-2xl text-muted-foreground font-manrope font-light leading-relaxed max-w-2xl mb-12">
              <HeroWords delay={heroEnd(9)}>
                Build enterprise-grade security solutions. Work on live
                projects. Get hired.
              </HeroWords>
            </p>
            ...
            >
              ...
              <Link
                href="/how-we-work"
                className="px-8 py-4 border border-border text-foreground font-semibold rounded-full hover:bg-muted/50 transition-colors"
              >
                Learn More
              </Link>
            </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-background/70">
                      Intensive bootcamp on core skills and tools.
                    </p>
                  </div>
                </div>
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-background/70">
                      Work on live modules under mentorship.
                    </p>
                  </div>
                </div>
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-background/70">
                      Performance review and internship certificate.
                    </p>
                  </div>
                </div>
              </div>
