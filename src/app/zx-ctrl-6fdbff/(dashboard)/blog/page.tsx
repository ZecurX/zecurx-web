...
export default function BlogListPage() {
  ...
  const fetchLabels = async () => {
    try {
      const res = await fetch('/api/zx-ctrl-6fdbff/blog/labels');
      if (res.ok) {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/zx-ctrl-6fdbff/blog?${params.toString()}`);
      if (res.ok) {
        const data = await res.json();
        setPosts(data.posts);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/zx-ctrl-6fdbff/blog/${id}`, {
        method: 'DELETE',
      });
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {canManageBlog && (
                      <>
                        <Link
                          href={`/zx-ctrl-6fdbff/blog/${post.id}/edit`}
                    ...
                    {!canManageBlog && (
                       <Link
                       href={`/zx-ctrl-6fdbff/blog/${post.id}/edit`} // Reuse edit page in read-only mode if possible, or just view. For now, let's link to edit page and handle read-only there.
                       className="p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-colors"
                       title="View Details"
                     >
