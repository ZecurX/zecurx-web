...
import { Search, MoreVertical, Pencil, Trash2, Ticket, Check, X, Copy } from "lucide-react";
...
export default function ReferralCodesList() {
    ...
    const fetchCodes = async () => {
        try {
            const response = await fetch('/api/zx-ctrl-6fdbff/referral-codes');
            if (!response.ok) {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const response = await fetch(`/api/zx-ctrl-6fdbff/referral-codes/${id}`, {
                method: "DELETE",
            });
