...
import { CreateWhitepaperRequest, WhitepaperStatus } from '@/types/auth';
...
export default function NewWhitepaperPage() {
  ...
  const handleSubmit = async (status: WhitepaperStatus) => {
    if (!title.trim()) {
    ...
    try {
      const payload: CreateWhitepaperRequest = {
        title,
        slug,
        summary,
        category,
      ...
      const res = await fetch('/api/zx-ctrl-6fdbff/whitepapers', {
        method: 'POST',
      ...
      if (!res.ok) {
        const error = await res.json();
        throw new Error(error.error || 'Failed to create whitepaper');
      }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold font-manrope text-foreground">New Whitepaper</h1>
                <p className="text-xs text-muted-foreground mt-0.5">Upload a technical whitepaper or research document</p>
              </div>
            </div>
              ...
              {pdfUrl ? (
                <div className="flex items-center gap-4 p-4 bg-green-500/10 border border-green-500/20 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-foreground truncate">{pdfFilename || 'PDF uploaded'}</p>
