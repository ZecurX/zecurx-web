...
import { CaseStudy, CaseStudyStatus } from '@/types/auth';
...
export default function CaseStudiesListPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(error.error || 'Failed to delete case study');
      }
    } catch (error) {
  ...
  const canManageCaseStudies = ['super_admin', 'admin', 'media', 'marketing'].includes(user?.role || '');
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage customer case studies and success stories</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 p-4 bg-card/40 border border-border/50 rounded-xl">
        ...
        <div className="flex gap-2">
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value as CaseStudyStatus | 'all')}
        ...
        <div className="text-center py-12 bg-card/40 border border-border/50 rounded-xl">
          ...
          <p className="text-muted-foreground mt-1">
            {search || statusFilter !== 'all' || categoryFilter !== 'all'
              ...
              : 'Get started by uploading your first case study'}
          </p>
        </div>
      ) : (
              ...
              <div className="flex-1 min-w-0 py-1">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${study.status === 'published'
                ...
                <div className="flex items-center gap-4 mt-4 text-xs text-muted-foreground">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span>{study.download_count} downloads</span>
                  </div>
                  <div className="truncate">
                    by {study.author_name || 'Unknown'}
                  </div>
                </div>
              </div>
            </div>
