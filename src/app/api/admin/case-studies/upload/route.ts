...
export async function POST(request: NextRequest) {
  const authResult = await requirePermission('case_studies', 'create', request);
  ...
  try {
    ...
    const folder = isPdf ? 'case-studies/pdfs' : 'case-studies/covers';
...
export async function DELETE(request: NextRequest) {
  const authResult = await requirePermission('case_studies', 'delete', request);
  if (!authResult.authorized) {
    return NextResponse.json({ error: authResult.error }, { status: authResult.status });
  }
  ...
  try {
    ...
    const result = await query(
      `SELECT COUNT(*) as count FROM case_studies WHERE pdf_url = $1 OR cover_image_url = $2`,
      [url, url]
    );
    ...
    if (isUsed) {
      return NextResponse.json({ 
        error: 'Cannot delete file that is currently used in case studies' 
      }, { status: 400 });
    }
