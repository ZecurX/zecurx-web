...
export async function GET(
    ...
    try {
        ...
        const leadResult = await query<EnterpriseLead>(
            `SELECT * FROM enterprise_leads WHERE id = $1`,
            [id]
        );
        ...
        const activitiesResult = await query(
            `SELECT * FROM enterprise_lead_activities WHERE lead_id = $1 ORDER BY created_at DESC`,
            [id]
        );

        const emailsResult = await query(
            `SELECT * FROM enterprise_lead_emails WHERE lead_id = $1 ORDER BY sent_at DESC`,
            [id]
        );
...
export async function PATCH(
    ...
    try {
        ...
        const oldLeadResult = await query<{ status: string; priority: string; assigned_to: string | null }>(
            `SELECT status, priority, assigned_to FROM enterprise_leads WHERE id = $1`,
            [id]
        );
        ...
        const result = await query<EnterpriseLead>(
            `UPDATE enterprise_leads SET ${updateFields.join(', ')} WHERE id = $${paramIndex} RETURNING *`,
            updateValues
        );
        ...
        for (const activity of activities) {
            await query(
                `INSERT INTO enterprise_lead_activities (lead_id, activity_type, description, old_value, new_value, performed_by)
                 VALUES ($1, $2, $3, $4, $5, $6)`,
...
export async function DELETE(
    ...
    try {
        ...
        await query(`DELETE FROM enterprise_leads WHERE id = $1`, [id]);
