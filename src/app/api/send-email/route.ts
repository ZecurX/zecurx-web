...
async function saveStudentLead(
  ...
  try {
    ...
    await query(
      ...
      [
        ...
        (body.courseTitle as string) ||
          (body.field_of_interest as string) ||
          "Cybersecurity",
...
async function saveEnterpriseLead(
  body: Record<string, unknown>,
  request: NextRequest,
  formType: string,
) {
  try {
    ...
    await query(
      ...
      [
        ...
        user_agent,
        formType === "seminar_booking"
          ? parseInt(body.attendees as string) || null
          : null,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const isInternship =
      isPurchase && body.itemName?.toLowerCase().includes("internship");
    ...
    const getGoogleCalendarUrl = (dateStr: string) => {
      ...
      const title = encodeURIComponent(
        isDemo ? "ZecurX Demo" : "ZecurX Meeting",
      );
      const details = encodeURIComponent(
        isDemo
          ? "Demo of ZecurX Security Platform."
          : "Meeting with ZecurX Team.",
      );
    ...
    const serviceToPdf: {
      [key: string]: { filename: string; cdnPath: string };
    } = {
      "penetration testing": {
        filename: "ZecurX_Penetration_Testing.pdf",
        cdnPath: "brochures/services/compressed/penetration-testing.pdf",
      },
      "vulnerability management": {
        filename: "ZecurX_Red_Teaming.pdf",
        cdnPath: "brochures/services/compressed/red-teaming.pdf",
      },
      "red teaming": {
        filename: "ZecurX_Red_Teaming.pdf",
        cdnPath: "brochures/services/compressed/red-teaming.pdf",
      ...
      "web & app security": {
        ...
        cdnPath: "brochures/services/compressed/security-ops.pdf",
      },
      ...
      "strategic consulting": {
        ...
        cdnPath: "brochures/services/compressed/risk-audit.pdf",
      },
      "risk audit": {
        filename: "ZecurX_Risk_Audit.pdf",
        cdnPath: "brochures/services/compressed/risk-audit.pdf",
    ...
    const selectedPdf =
      serviceToPdf[selectedService] || serviceToPdf["penetration testing"];
    ...
    if (isBrochure && body.brochureLink) {
      ...
      try {
        ...
        // Brochure PDF not available on CDN — continue without attachment
      }
    }
    ...
    if (isDemo) userSubject = "Demo Request Received - ZecurX";
      ...
      userSubject = `Your ${body.courseTitle} Brochure - ZecurX`;
    else if (isInternship)
      userSubject = `Internship Enrollment Confirmed: ${body.itemName} - ZecurX`;
    else if (isPurchase)
      userSubject = `Order Confirmation: ${body.itemName} - ZecurX`;
    else if (isSeminarBooking)
      userSubject = `Seminar Booking Request Received - ZecurX`;
    else if (isContact && preferredDate)
      userSubject = "Meeting Request Received - ZecurX";
