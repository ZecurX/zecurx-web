...
const useBreakableCard = (onBreak?: () => void) => {
    ...
    const debrisIdCounter = useRef(0); // Unique counter for debris IDs
...
const DebrisChunk = ({ x, y, rot, path }: DebrisChunkProps) => {
    ...
    return (
        ...
        >
            <path d={path} fill="black" className="dark:fill-white" stroke="white" strokeWidth="2" />
        </svg>
    );
};
...
const CrackLines = ({ level }: { level: number }) => {
    ...
    return (
        <svg className="absolute inset-0 w-full h-full pointer-events-none overflow-visible z-20" viewBox="0 0 100 100" preserveAspectRatio="none">
            {level >= 0.1 && <path d="M0 20 L15 25 L8 40 L20 50" fill="none" stroke="currentColor" className="text-black dark:text-white" strokeWidth="2" vectorEffect="non-scaling-stroke" style={{ opacity: Math.min(1, level * 2) }} />}
            {level >= 0.5 && <path d="M50 100 L55 80 L45 70 L60 55" fill="none" stroke="currentColor" className="text-black dark:text-white" strokeWidth="2" vectorEffect="non-scaling-stroke" style={{ opacity: Math.min(1, (level - 0.4) * 2) }} />}
            {level >= 0.9 && <path d="M20 0 L25 20 L40 25 L35 45 L50 50" fill="none" stroke="currentColor" className="text-black dark:text-white" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeDasharray="4 2" />}
        </svg>
    );
};
 ...
 * =========================================================================
 */
export const BreakableCard = ({ children, className, onBreak }: SectionCardProps) => {
    const {
        isDragging,
        position,
        crackLevel,
    ...
    const reactId = useId();
    const cardId = `card-${reactId.replace(/:/g, '')}`;
    ...
    // To prevent native drag on inner elements during our drag interaction
    ...
    return (
        <div className={cn("relative w-full h-full", className)}>
            ...
            */}
            <div className="absolute inset-0 bg-muted/20 border-2 border-dashed border-border flex flex-col items-center justify-center z-0 rounded-3xl">
                <span className="text-muted-foreground font-semibold text-xl uppercase tracking-widest mb-2 opacity-50">Shattered</span>
                {/* Loading Bar (Visible only when waiting for respawn) */}
                {isBroken && (
                    <div className="w-32 h-2 rounded-full overflow-hidden bg-muted relative mt-2">
                        <div
            ...
            */}
            <div
                ...
                onClick={(e) => {
                    // Prevent click events if we are or were just dragging
                ...
                className={cn(
                    "relative z-10 w-full h-full overflow-hidden transition-[background-color] rounded-3xl",
            ...
            >
                {/* The ZecurX Card passed as Child */}
                <div className="w-full h-full relative pointer-events-none *:pointer-events-auto select-none" draggable={false}>
                    {children}
                </div>
                ...
                {crackLevel > 0.2 && (
                    <div className="absolute inset-0 bg-red-500 mix-blend-multiply pointer-events-none z-50 rounded-3xl" style={{ opacity: crackLevel * 0.2 }} />
                )}
                {isFlashing && (
                    <div className="absolute inset-0 bg-white/30 mix-blend-overlay pointer-events-none z-50 rounded-3xl" />
                )}
            </div>
